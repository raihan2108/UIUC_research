...
for mt in range(0,M):
	...
	for i in range(n):
		dict_pr[pr[i]] = i
	for ratio in np.arange(link1,link2,step):
		...
		SD_ancestor = Generate_graph(n,ratio)
		#---------------------
		for s in range(n):
...
fw_err = open('Rand_Link_error_rand.txt','w')
fw_err.write(str(Err1)+'\n'+str(Err2)+'\n'+str(Err3)+'\n'+str(Err4)+'\n')
fw_err.close()

print("when the links ratio random,cost,crowdbudget:")
print(Err1)
print(Err2)
print(Err3)
print(Err4)
