...
def Get_graph(Src_num_dict):
	...
	fw_child_par = open('Output_social_rt.txt','w')
	SD_ancestor = dict()
	# SD_successor =dict()
	First_src = []
	for lines in fr_graph.readlines():
		...
		fw_child_par.write(str(father)+'\t'+str(child)+'\n')
...
for Nt in range(1, 2, 2):
	for mt in range(itera):  # the number of iteration
		...
		sai = np.round(ai*100)/100
		sbi = np.round(bi*100)/100
		fi = np.random.uniform(0.5,0.9,N)
		gi = np.random.uniform(0.1,0.4,N)
		sfi = np.round(fi*100)/100
		sgi = np.round(gi*100)/100
		...
		qi = Get_qi(NumV,N,st_fst_id,ai)
		# print(qi)
		hi = np.random.uniform(0.8,0.9,N)
		hi = np.round(hi,decimals=3)
		Parameter = np.vstack((sai,sbi,sfi,sgi))  # parameters for EM algorithm
...
for rst in Z_rst:
	fr_opinionEM.write(str(rst)+'\n')
# fw_sc = open('SC_rst.txt','w')
# for j in range(NumV):
# 	num1 = sum(SC[j,:])
# 	fw_sc.write(str(SC[j,:])+'\n')
