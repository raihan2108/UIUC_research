...
#  -------------------------------------------------------
fread_ids = open('Rand_ids.txt','r')  # notice the order of the sources
i = 0
for ids in fread_ids.readlines():
	org_id = []
	i +=1
	fw_author_claim = open(str(i)+'rand_author_claim.txt','w')
...
for j in range(1,num_txt):
	claim_order=[]
	claim_dict = dict()
	read_author_claim = open(str(j)+'rand_author_claim.txt','r')
	...
	fw_user_claim = open(str(j)+'rand_SCj.txt','w')
