...
we calculate the expected error by changing the [Change costS among sources]
'''
	...
	for s in Org_ids:
		...
		if s in SD_ancestor.keys():
			...
			if len(father) > 0:
				for f in father:
					sum_ai_anct += ai[f]
					sum_bi_anct += bi[f] #notice I change this
...
def fun_window6(Rai,Rbi,C_credit):
	...
	# print(ST_all)
	# print(C_credit)
	print("----it is running and change cost--",len(Rai))
	for Pro in C_credit:
		Sum = 0
...
for mt in range(0,M):
	...
	for i in range(n):
		dict_pr[pr[i]] = i
	##--------****---------------
	for cost in np.arange(w1,w2,step):
		ai = S_ai
		bi = S_bi
		pr = deepcopy(s_prc)
		...
		err2 = rand_fun(n,S_ai,S_bi,cost,pr,C_credit,SD_ancestor)   #same as
...
fw_err = open('Cost_error_reb.txt','w')
fw_err.write(str(Err1)+'\n'+str(Err2)+'\n'+str(Err3)+'\n'+str(Err4)+'\n')
fw_err.write("cost reb changes from 20-70")
fw_err.close()


print("when cost changes from 20 to 70, we have")
print(Err1)
print(Err2)
print(Err3)
print(Err4)
