...
Functions: Evaluation based on Twitter data and their reliability
...
[4] c_credit.txt lists the assertion credibilities
'''
...
def fun_window6(Rai,Rbi,C_credit):
	...
	print("it is running SLE algorithm")
	C=[C1, C2, C3, C4, C5, C6]
	B=[B1, B2, B3, B4, B5, B6]
	mc = np.zeros(delta)
	mb = np.zeros(delta)
...
def crowdbudget(prc,cost,R_ai,R_bi,C_credit):
	...
	# print(dm1,dm2)
	for pro_asst in C_credit:
		err=0.5*np.exp(-2*N*dm1**2)+(0.5)*np.exp(-2*N*dm2**2)
		crowd_err += err
...
def Pro_assertion():
	C_credit =[]
	credit_dict = dict()
	f_assertion = open('c_credit.txt','r')
...
rst_err3 = []
rst_err4 = []
file_wt_ids = open("Opt_ids.txt",'w')
rand_wt_ids = open("Rand_ids.txt",'w')
cost_wt_ids = open("Cost_ids.txt",'w')
...
for mt in range(0,M):
	...
	for cost in range(cst1,cst2,10):
		...
		n = len(ai)
		spr = np.random.uniform(0.5, pri, n)  # generate the prices for source
		spr = np.around(spr,decimals=3)
		# ---below fun: store the [ai, id]
		#----below fun: sort the reliability of sources
		Hurist_rab = 1 + ai - bi+np.random.randn(n)*0.0000001 #if ai and bi are the same, it is hard to find
		# print(Hurist_rab)
		# print(Hurist_rab)
		rab_dict = dict()
		for i in range(n):
		...
		sort_pr = np.sort(spr)
		sort_pr = sorted(sort_pr,reverse=True)
		...
		for j in range(n):
			abi = Rab[j]
			ids = rab_dict[abi]
			pr[ids] = sort_pr[j]  #give the source the price based on reb
			new_Ra[j] = ai[ids]
			new_Rb[j] = bi[ids]
			ids_dict[j] = ids
		new_price = deepcopy(sort_pr)
		dict_pr = dict()
		for i in range(n):
			dict_pr[pr[i]] = i
		...
		err2,rand_ids = rand_fun(n,new_Ra,new_Rb,cost,new_price,C_credit)   #same as the RA,RB in the Matlab codes
		err3,cost_ids = cost_only(Ra_prc,Rb_prc,ct,cost,C_credit)
		# print(len(crowd_ai))
		err4 = crowdbudget(crowd_price,cost,crowd_ai, crowd_bi,C_credit)

		ids_rand=[]	#store the random ids
		for ids in rand_ids:
			myid = ids_dict[ids]
			ids_rand.append(myid)

		ids_cost =[]
		for ids in cost_ids:
			myid = cost_id_dict[ids]
			ids_cost.append(myid)
		...
		ID_rand.append(ids_rand)
		ID_cost.append(ids_cost)
	...
	rand_wt_ids.write(str(ID_rand)+'\n')
	cost_wt_ids.write(str(ID_cost)+'\n')
...
f_Err = open("Error_compare.txt",'w')
f_Err.write(str(Err1)+'\t'+str(Err2)+'\t'+str(Err3)+'\t'+str(Err4))
f_Err.close()
