...
for j in range(2, 16, 2):
	file_std = open("res-q"+str(j)+"-stat-e40.txt",'r')	# standard file
	for num, line in enumerate(file_std):
		line = line.split()
		cache = int(line[0]) + int(line[1])
		web = j/5.0	# five severs
		cache = cache/4.0
		db = float(line[2])
...
web_request = np.divide(sum_web, 1)
...
for k in range(2, 16, 2):
	...
	sum_db_delay = 0.0
	sum_web_delay = 0.0
	file_cache_delay = open("res-q"+str(k)+"-cn-e40.txt",'r')
	file_db_delay = open("res-q"+str(k)+"-dn-e40.txt",'r')
	file_web_delay = open("res-q"+str(k)+"-wn-e40.txt",'r')
...
arr_one = -np.ones(7)   		# get seven ones array for the equations
